...
- UI Layout: FormKit header + section cards (Applicant, Travel, Medical, Emergency, Declaration) on a dark theme; status pill + meta (id, created_at) in header; hero thumbnails for photo/form with add buttons; Quick Edit uses FormKit inputs with medical grid + shared notes textarea (heart/bp/diabetes/asthma) and other-condition notes; datetime picker; review actions in header.
...
- Validation: Upload buttons disabled during upload; name/address/phone/declaration required on save; status limited to enum; shared medical notes disabled/cleared when no medical flags; single photo/form path per registration.
